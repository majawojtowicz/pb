...
## 9. Jak dodać nowy typ atrakcji

> **To kluczowa cecha systemu.** Dodanie nowego rodzaju atrakcji (np. `AquaparkSpec`, `EscapeRoomSpec`)
> wymaga tylko **jednej nowej klasy Java**. Żaden istniejący kod nie jest modyfikowany.
> System nie wymaga restartu ani redeploymentu dla nowych instancji znanych typów.

### Krok 1 – stwórz klasę w pakiecie `com.attractions.spec.domain`

```java
public class AquaparkSpec extends AttractionSpec {
    private int numberOfSlides;
    private boolean hasWavePool;
    private LocalTime openFrom;
    private LocalTime openTo;

    @Override public String getType() { return "AQUAPARK"; }
    ...
    public int getMaxCapacity() {
        return numberOfSlides * 50 + (hasWavePool ? 200 : 0);
    }

    @Override
    public void validate() {
        if (numberOfSlides <= 0) throw new SpecificationException("Brak zjeżdżalni");
    }
}
```

### Krok 2 – zarejestruj w `DataInitializer`

```java
registry.register(new AquaparkSpec());
```

### Krok 3 – dodaj archetyp przez REST API (bez redeploymentu)
...
POST /api/attractions
{
  "name": "Aquapark Fala",
  "category": "AQUAPARK",
  "tags": ["sport", "dzieci", "lato"],
  "description": "Największy aquapark w regionie"
}
```

### Krok 4 – dodaj do katalogu
...
Nowa atrakcja jest gotowa. Nie było potrzeby modyfikacji żadnej istniejącej klasy.
